      ...
      while(1){
        ...
        if(rc < 0){
          ...
          if(FD_ISSET(conn_fd,&readfds)){
            protocol p;
            int bytes_recv = recv(conn_fd,str,sizeof(str),0);
            // printf("Nhan: %s\n",str);//CHECK ND BAN TIN DC GUI TU PHIA CLIENT
            toObject(&p,str);
            if(bytes_recv < 0){
              // Recv() error
              ...
              if(state == CONNECTED){
                if(!strcmp(p.p_message,WANT_TO_SIGNUP)){
                  p.p_state = SIGNUP;
                  strcpy(p.p_message,ALLOW_TO_SIGNUP);
                } else if(!strcmp(p.p_message,WANT_TO_SIGNIN)){
                  p.p_state = UNAUTHENTICATE;
                  strcpy(p.p_message,ALLOW_TO_SIGNIN);
                }
